...
/** A quiet day and the real 2026-09-17 shape (25 rounds, 20 players, 4 games). */
...
describe("statsCsv", () => {
  ...
  test("header row then one row per day, oldest first", () => {
    ...
    expect(rows[2]).toBe(
      "2026-09-17,41,12,30,25,20,33.3,88.4,scan-rush 19; ecg-rhythm 3; memory-scan 2; bone-buster 1",
    );
  ...
  test("a game id holding a comma or a quote is escaped, row stays 9 fields", () => {
    ...
    expect(row).toBe('2026-09-18,0,0,0,2,0,0,,"word ""search"", 2 1; bone-buster 1"');
  });
